...
function baseUrl(provider, env) {
  ...
  const value = provider.base_url || (provider.base_url_env && env[provider.base_url_env]) || provider.default_base_url;
...
export function startAttempt({ candidate, req, path, body, env = process.env, signal }) {
  ...
  if (candidate.provider.adapter === "oauth-proxy") headers.authorization = `Bearer ${env[candidate.provider.internal_api_key_env]}`;
  else if (candidate.provider.adapter === "openai-compatible") headers.authorization = `Bearer ${env[candidate.provider.api_key_env]}`;
